...
public class AlbumDatabase {
  ...
  public static List<AlbumCollection> makeAlbumCollection(String fileName) {
    ...
    List<AlbumCollection> albumCollections = new ArrayList<>(); // creates a list of AlbumCollections that're grouped by
                                                                // artist
    ...
    try (BufferedReader reader = new BufferedReader(new FileReader(fileName))) {
      ...
      while ((line = reader.readLine()) != null) {
        ...
        // checks if the line contains album info in the format || <Artist> :
        // <AlbumTItle> (<Year>)
  ...
  public static void two(List<AlbumCollection> ourCollection) {
    System.out.println("all albums soted in alphabetical order and ascending order: \n");
  ...
  public static int findInsertionIndex(List<Album> sortedAlbums, Album currentAlbum) {// compares two given albums by
                                                                                      // artist, then year if needed
    for (int index = 0; index < sortedAlbums.size(); index++) {// iterate over sortedAlbums
      if (compareAlbums(currentAlbum, sortedAlbums.get(index)) != 0) {// comparing the current album and the album at
                                                                      // index i for the sortedAlbums
        return index;
      } // end of if
    } // end of for loop
  ...
  public static int compareAlbums(Album albumA, Album albumB) { // compares two given albums by artist, then year if
                                                                // needed
  ...
  public static void three(
    ...
    String formattedTime = (hours < 10 ? "0" : "") +
        hours +
        ":" +
        (minutes < 10 ? "0" : "") +
        minutes +
    ...
    for (AlbumCollection coll : ourCollection) {
      ...
      for (Album album : Albums) {
        if (shortestTitleAlbum == null ||
            album.getAlbumTitle().length() < shortestTitleAlbum.getAlbumTitle().length()) {
      ...
      for (Album album : Albums) {
        for (Track track : album.getTracks()) {
          if (longestTrack == null ||
              track.getDuration().getDurationLength() > longestTrack.getDuration().getDurationLength()) {
            longestTrack = track;
